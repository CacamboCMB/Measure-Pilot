...
def test_rectangle_and_two_circular_holes_are_classified() -> None:
    ...
    centers = [hole["center_mm"] for hole in report["features"]["circular_holes"]]
...
def test_non_circular_hole_is_retained_as_unresolved() -> None:
    ...
    assert any(item["code"] == "NON_CIRCULAR_HOLE" for item in result.report["unresolved"])
...
def test_empty_mask_is_rejected() -> None:
    with pytest.raises(AnalysisError, match="empty"):
        extract_features(np.zeros((100, 100), dtype=np.uint8), px_per_mm=PX_PER_MM)
