...
M2 deliberately supports one dominant planar foreground component.  It does
not infer hidden geometry and it does not silently choose between ambiguous
...
def _valid_region(
    ...
    for marker_id in MARKER_IDS:
        ...
        x0 = max(0, int(math.floor((float(corners[:, 0].min()) - extra) * config.px_per_mm)))
        y0 = max(0, int(math.floor((float(corners[:, 1].min()) - extra) * config.px_per_mm)))
        x1 = min(width, int(math.ceil((float(corners[:, 0].max()) + extra) * config.px_per_mm)))
        y1 = min(height, int(math.ceil((float(corners[:, 1].max()) + extra) * config.px_per_mm)))
        valid[y0:y1, x0:x1] = 0
    return valid
...
def _touches_invalid_region(component: np.ndarray, valid: np.ndarray) -> bool:
    boundary = cv2.dilate((valid == 0).astype(np.uint8), np.ones((3, 3), np.uint8))
...
def _component_candidate(
    ...
    *,
    ...
    if _touches_invalid_region(selected, valid):
        raise AnalysisError(f"dominant {polarity} component touches an excluded boundary")
    ...
    # Prefer a decisive dominant component, then a useful but non-page-filling area.
...
def segment_foreground(
    ...
    *,
    ...
    if not candidates:
        raise AnalysisError("auto polarity found no valid foreground; " + "; ".join(failures))
    if len(candidates) == 1:
