...
def _points_mm(points_px: np.ndarray, px_per_mm: float) -> list[list[float]]:
    return [
        [
            round(float(x) / px_per_mm, 6),
            round(float(y) / px_per_mm, 6),
        ]
...
def _polygon_rms_distance(points: np.ndarray, polygon: np.ndarray) -> float:
    vertices = polygon[:-1] if np.array_equal(polygon[0], polygon[-1]) else polygon
    squared: list[float] = []
    for point in points:
        distance = min(
...
def _line_features(
    ...
    vertices = (
        simplified_px[:-1]
        if np.array_equal(simplified_px[0], simplified_px[-1])
        else simplified_px
    )
    ...
    fit_confidence = max(
        0.0,
        min(1.0, 1.0 - rms_mm / config.simplify_tolerance_mm),
    )
    ...
    for index in range(len(vertices)):
        ...
        angle_deg = math.degrees(
            math.atan2(float(delta_mm[1]), float(delta_mm[0]))
        )
        if angle_deg < 0.0:
            angle_deg += 360.0
        evidence = {
            "end_mm": [
                round(float(end[0]) / px_per_mm, 6),
                round(float(end[1]) / px_per_mm, 6),
            ],
            "length_mm": round(length_mm, 6),
            "source_edge_index": index,
            "start_mm": [
                round(float(start[0]) / px_per_mm, 6),
                round(float(start[1]) / px_per_mm, 6),
            ],
        }
            ...
            unresolved.append(
                {
                    ...
                    "evidence": evidence,
                    "message": (
                        "simplified outer edge is too short for line classification"
                    ),
                }
            )
            continue
        length_confidence = min(
            1.0,
            length_mm / (config.min_line_length_mm * 3.0),
        )
        lines.append(
            {
                "angle_deg": round(angle_deg, 6),
                "confidence": round(
                    fit_confidence * length_confidence,
                    6,
                ),
...
def _hole_features(
    ...
    for index, contour_px in enumerate(hole_contours):
        open_points = (
            contour_px[:-1]
            if np.array_equal(contour_px[0], contour_px[-1])
            else contour_px
        )
        ...
        if area_mm2 < config.min_hole_area_mm2:
            ...
            continue
        try:
            center_x, center_y, radius, residual = _fit_circle(
                open_points / px_per_mm
            )
        ...
        residual_score = max(
            0.0,
            min(1.0, 1.0 - residual / residual_limit),
        )
            ...
            unresolved.append(
                {
                    ...
                    "message": (
                        "enclosed contour is retained but not classified as a "
                        "circular hole"
                    ),
                }
            )
        ...
        circles.append(
            {
                ...
                "circularity": round(circularity, 6),
                "confidence": round(
                    0.5 * residual_score + 0.5 * circularity_score,
                    6,
                ),
                "contour_id": f"hole-contour-{index:03d}",
...
def extract_features(
    ...
    *,
    ...
    if len(external) != 1:
        raise AnalysisError(
            "foreground mask must contain exactly one external contour"
        )
    outer_index = external[0]
    outer_raw = np.asarray(
    ...
    canonical_holes = tuple(
        _canonical_points(
            np.asarray(contours[index], dtype=np.float64).reshape(-1, 2)
        )
    ...
    report: dict[str, Any] = {
        "contours": {
            ...
            "outer": {
                ...
                "simplified_points_mm": _points_mm(
                    simplified_px,
                    px_per_mm,
                ),
                "simplification_rms_mm": round(
                    simplification_rms_mm,
                    6,
                ),
            },
        },
