    ...
    return {
        ...
        "segmentation": {
            ...
            "marker_exclusion_margin_mm": (
                segmentation.marker_exclusion_margin_mm
            ),
            "max_foreground_fraction": segmentation.max_foreground_fraction,
            "max_secondary_area_ratio": (
                segmentation.max_secondary_area_ratio
            ),
...
def _draw_overlay(
    ...
    simplified = (
        np.rint(features.simplified_outer_px)
        .astype(np.int32)
        .reshape(-1, 1, 2)
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    features = FeatureConfig(
        simplify_tolerance_mm=arguments.simplify_tolerance_mm
    )
    try:
        report = analyze_file(
            arguments.input,
